    ...
    implements Map<K,V>
{
    ...
    **/
    public Set<Entry<K,V>> entrySet()
    {
        ...
        /* Map.Entry map */
        Set<Entry<K,V>> es = super.entrySet(); // unordered
        Map<K,Entry<K,V>> meMap = new HashMap<K,Entry<K,V>>();
        for (Iterator<Entry<K,V>> i = es.iterator(); i.hasNext();) {
            Entry<K,V> me = i.next();
            K key = me.getKey();
            meMap.put(key, me);
        }
        ...
        /* place in keyOrder */
        OrderedSet<Entry<K,V>> entSet = new OrderedSet<Entry<K,V>>();
        for (Iterator<K> i = this.keyOrder.iterator(); i.hasNext();) {
            K key = i.next();
            Entry<K,V> me = meMap.get(key);
